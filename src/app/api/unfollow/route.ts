...
import { isEventInHubSyncScope } from '@/lib/hubGcal';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const userEventSyncs = await prisma.userEventSync.findMany({
      where: {
        userId: session.user.id,
        eventId,
      },
    ...
    for (const userEventSync of userEventSyncs) {
      let shouldRemove = false;

      if (event?.hubId) {
        const stillInScope = await isEventInHubSyncScope(
          session.user.id,
          event.hubId,
          eventId
        );
        shouldRemove = !stillInScope;
      } else if (
        dbUser?.gcalCalendarId &&
        dbUser.gcalSyncMode === 'CUSTOM' &&
        userEventSync.gcalCalendarId === dbUser.gcalCalendarId
      ) {
        shouldRemove = true;
      }

      if (shouldRemove && googleAccount?.access_token) {
        try {
          const iCalUID = generateEventICalUID(eventId);
          await deleteEventFromGoogleCalendar(
            googleAccount.access_token,
            googleAccount.refresh_token || undefined,
            userEventSync.gcalCalendarId,
            iCalUID
          );
          gcalRemoved = true;
        } catch (error) {
